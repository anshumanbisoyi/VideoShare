const MEETING_SERVICE =
  "https://ayvjdyzi3c.execute-api.us-east-1.amazonaws.com/bytes-meeting";
...
var meetItems = document.querySelector(".meet-items");
var meetImage = document.querySelector(".meet-image");
...
async function doMeeting() {
  ...
  meetItems.style.display = "block";
  meetImage.style.display = "none";
...
function updateTiles(meetingSession) {
  ...
  tiles.forEach((tile) => {
    ...
    if (!divElement) {
      ...
      tileUserName.style.color = "green";
...
async function stopMeeting() {
  ...
  meetItems.style.display = "none";
  meetImage.style.display = "flex";
  try {
    var response = await fetch(requestPath, {
      method: "POST",
      headers: new Headers(),
